...
from Motor_code.encoderClass import SimpleEncoder, BetterEncoder
...
class Sys5_Control:
    ...
    def __init__(self) -> None:
        ...
        self.rot =0 
        self.State = None 
        self.EncoderL = BetterEncoder(motor1cha,motor1chb) # set up Left Motor
        self.EncoderR = BetterEncoder(motor2cha,motor2chb) # set up right Motor
    ...
    def GoTo(self,X:float,Y:float,speed:float = 30)->None:
        ...
        try:
            ...
            if (angle >-1 and angle <1): # no rotation required 
                ...
                numPulses = distance/GLOBALSM1.distancePerPulse+abs(self.EncoderL.steps)
            else: 
                self.State=self._turn(speed,True)# rotatte CW
                numPulses = -distance/GLOBALSM1.distancePerPulse+abs(self.EncoderL.steps) 
            
            while (abs(self.EncoderL.steps -numPulses)>500):
            ...
            encoderOldCount = self.EncoderL.steps # update encoder count 
            ...
            while (self.EncoderL.steps >numPulses):# keep going forwards until you reach the desired number of pulses 
                self.x_pos, self.y_pos, self.rot = self._updatePos(self.x_pos,self.y_pos,self.rot)
                sleep(0.5)
            self._stop()
    ...
    def Home(self)->None:
        try:
            ...
            if (angle >-1 and angle <1): # no rotation required 
                ...
                numPulses = (distance/GLOBALSM1.distancePerPulse)+self.EncoderL.steps
            else: 
            ...
            encoderOldCount = self.EncoderL.steps
            numPulses = -(distance*1.05/GLOBALSM1.distancePerPulse)+encoderOldCount
            self.State = self._forwards(speed)

            while (self.EncoderL.steps >numPulses):# keep going fowards until you reach the desired number of pulses 
                self.x_pos, self.y_pos, self.rot = self._updatePos(self.x_pos,self.y_pos,self.rot)
                sleep(0.02)
            self._stop
            GPIO.cleanup()
            ...
            while(direction == DIRECTION.CannotFind):
                ...
                if (self.rot >= 90):
                    ...
                    if (angle >-1 and angle <1): # no rotation required 
                        ...
                        numPulses = distance/GLOBALSM1.distancePerPulse+self.EncoderL.steps # calculate the number of pulses achieve this rotation
                    else: 
